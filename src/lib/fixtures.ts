import type { CodexResetsStatus, DashboardState } from "./contracts";

export const previewState: DashboardState = {
  mode: "preview",
  refreshedAt: "2026-08-25T13:42:00Z",
  settings: {
    showMenuBarItem: true,
    updateChannel: "stable",
    telemetryEnabled: true,
    traySummary: { connectionId: "claude-work", windowKey: "five_hour" },
  },
  connections: [
    {
      id: "claude-work",
      provider: "claude",
      label: "Claude · work@example.com",
      sourceLocator: "oauth/claude/u-1",
      enabled: true,
      status: "ready",
      source: "Claude subscription API",
      lastUpdatedAt: "2026-08-25T13:42:00Z",
      customLabel: "Work",
      identity: { displayName: "work@example.com", plan: "Max 5x" },
      alerts: { lowQuota: true, resetSoon: true, resetHappened: true },
      autoPing: {
        enabled: true,
        lastPingAt: "2026-08-25T10:00:03Z",
      },
      windows: [
        {
          key: "five_hour",
          label: "Session (5h)",
          usedPercent: 63,
          resetsAt: "2026-08-25T15:00:00Z",
        },
        {
          key: "seven_day",
          label: "Weekly",
          usedPercent: 28,
          resetsAt: "2026-08-29T07:00:00Z",
        },
        {
          key: "seven_day_fable",
          label: "Fable",
          usedPercent: 12,
          resetsAt: "2026-08-29T07:00:00Z",
        },
        {
          key: "extra_usage",
          label: "Extra usage",
          usedPercent: 100,
          amount: { used: 600.34, total: 600, unit: "USD" },
          paid: true,
        },
      ],
    },
    {
      id: "claude-personal",
      provider: "claude",
      label: "Claude CLI · Default",
      sourceLocator: "~/.claude",
      enabled: true,
      status: "stale",
      source: "Claude subscription API (cached)",
      lastUpdatedAt: "2026-08-25T13:28:00Z",
      identity: { displayName: "me@example.com", plan: "Pro" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "five_hour",
          label: "Session (5h)",
          usedPercent: 18,
          resetsAt: "2026-08-25T16:00:00Z",
        },
        {
          key: "seven_day",
          label: "Weekly",
          usedPercent: 46,
          resetsAt: "2026-08-30T07:00:00Z",
        },
      ],
    },
    {
      id: "claude-team",
      provider: "claude",
      label: "Claude · team@example.com",
      sourceLocator: "oauth/claude/u-3",
      enabled: true,
      status: "needs_login",
      source: "Claude subscription API",
      lastUpdatedAt: "2026-08-24T09:12:00Z",
      lastError: "The Claude login expired. Sign in again.",
      identity: { displayName: "team@example.com", plan: "Max 5x" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "five_hour",
          label: "Session (5h)",
          usedPercent: 0,
          resetsAt: "2026-08-24T14:00:00Z",
        },
        {
          key: "seven_day",
          label: "Weekly",
          usedPercent: 0,
          resetsAt: "2026-08-29T07:00:00Z",
        },
      ],
    },
    {
      id: "codex-default",
      provider: "codex",
      label: "Codex CLI · Default",
      sourceLocator: "~/.codex",
      enabled: true,
      status: "ready",
      source: "Codex local records",
      lastUpdatedAt: "2026-08-25T13:41:00Z",
      identity: { plan: "Plus" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "primary",
          label: "Session (5h)",
          usedPercent: 41,
          resetsAt: "2026-08-25T15:34:00Z",
        },
        {
          key: "secondary",
          label: "Weekly",
          usedPercent: 71,
          resetsAt: "2026-08-28T09:00:00Z",
        },
        {
          key: "credits",
          label: "Credits",
          usedPercent: 0,
          amount: { total: 12, unit: "credits" },
          paid: true,
        },
      ],
      resetCredits: [
        {
          id: "RateLimitResetCredit_preview-1",
          title: "Rate limit reset",
          grantedAt: "2026-08-20T09:00:00Z",
          expiresAt: "2026-09-04T14:00:00Z",
        },
      ],
    },
    {
      id: "codex-team",
      provider: "codex",
      label: "Codex · team@example.com",
      sourceLocator: "oauth/codex/acc-2",
      enabled: true,
      status: "ready",
      source: "ChatGPT subscription API",
      lastUpdatedAt: "2026-08-25T13:41:00Z",
      identity: { displayName: "team@example.com", plan: "Team" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "primary",
          label: "Session (5h)",
          usedPercent: 0,
          resetsAt: "2026-08-25T18:42:00Z",
        },
        {
          key: "secondary",
          label: "Weekly",
          usedPercent: 0,
          resetsAt: "2026-09-01T13:42:00Z",
        },
        {
          key: "monthly_credits",
          label: "Monthly credits",
          usedPercent: 32,
          resetsAt: "2026-09-18T01:00:00Z",
          amount: { used: 640, total: 2000, unit: "credits" },
          paid: true,
        },
        {
          key: "credits",
          label: "Credits",
          usedPercent: 0,
          amount: { total: 10, unit: "credits" },
          paid: true,
        },
      ],
    },
    {
      id: "codex-credits-only",
      provider: "codex",
      label: "Codex · member@example.com",
      sourceLocator: "oauth/codex/acc-3",
      enabled: true,
      status: "ready",
      source: "ChatGPT subscription API",
      lastUpdatedAt: "2026-08-25T13:41:00Z",
      identity: { displayName: "member@example.com", plan: "Team" },
      alerts: { lowQuota: true, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "credits",
          label: "Credits",
          usedPercent: 0,
          paid: true,
        },
      ],
    },
    {
      id: "cursor-team",
      provider: "cursor",
      label: "Cursor · work@example.com",
      sourceLocator: "oauth/cursor/user_01",
      enabled: true,
      status: "ready",
      source: "Cursor dashboard API",
      lastUpdatedAt: "2026-08-25T13:40:00Z",
      identity: { displayName: "work@example.com", plan: "Enterprise" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "cursor_models",
          label: "Cursor Models",
          usedPercent: 12,
          resetsAt: "2026-08-29T00:00:00Z",
        },
        {
          key: "other_models",
          label: "Other Models",
          usedPercent: 100,
          resetsAt: "2026-08-29T00:00:00Z",
        },
        {
          key: "on_demand",
          label: "On-demand",
          usedPercent: 100,
          resetsAt: "2026-08-29T00:00:00Z",
          amount: { used: 50, total: 50, unit: "USD" },
          paid: true,
        },
        {
          key: "pooled",
          label: "Team pool",
          usedPercent: 0,
          resetsAt: "2026-08-29T00:00:00Z",
          unlimited: true,
          paid: true,
        },
      ],
    },
    {
      id: "gemini-personal",
      provider: "gemini-cli",
      label: "Gemini CLI · me@example.com",
      sourceLocator: "oauth/gemini-cli/google-1",
      enabled: true,
      status: "ready",
      source: "Gemini Code Assist API",
      lastUpdatedAt: "2026-08-25T13:39:00Z",
      identity: { displayName: "me@example.com", plan: "Free" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "gemini-3.1-pro-preview",
          label: "Gemini 3.1 Pro Preview",
          usedPercent: 35,
          resetsAt: "2026-08-25T17:00:00Z",
        },
        {
          key: "gemini-3-flash",
          label: "Gemini 3 Flash",
          usedPercent: 12,
          resetsAt: "2026-08-25T17:00:00Z",
        },
      ],
    },
    {
      id: "antigravity-personal",
      provider: "antigravity",
      label: "Antigravity · me@example.com",
      sourceLocator: "oauth/antigravity/google-1",
      enabled: true,
      status: "ready",
      source: "Antigravity API (Unofficial)",
      lastUpdatedAt: "2026-08-25T13:39:00Z",
      identity: { displayName: "me@example.com", plan: "Google AI Pro" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "gemini-5h",
          label: "Gemini (5h)",
          usedPercent: 60,
          resetsAt: "2026-08-25T18:00:00Z",
        },
        {
          key: "gemini-weekly",
          label: "Gemini (Weekly)",
          usedPercent: 22,
          resetsAt: "2026-08-30T13:00:00Z",
        },
        {
          key: "3p-5h",
          label: "Other (5h)",
          usedPercent: 0,
          resetsAt: "2026-08-25T18:00:00Z",
        },
        {
          key: "3p-weekly",
          label: "Other (Weekly)",
          usedPercent: 8,
          resetsAt: "2026-08-30T13:00:00Z",
        },
      ],
    },
    {
      id: "copilot-octocat",
      provider: "copilot",
      label: "GitHub Copilot · octocat",
      sourceLocator: "github.com/octocat",
      enabled: true,
      status: "ready",
      source: "GitHub Copilot API",
      lastUpdatedAt: "2026-08-25T13:40:00Z",
      identity: { displayName: "octocat", plan: "Pro" },
      alerts: { lowQuota: false, resetSoon: false, resetHappened: false },
      autoPing: { enabled: false },
      windows: [
        {
          key: "premium_interactions",
          label: "AI Credits",
          usedPercent: 45,
          resetsAt: "2026-09-01T00:00:00Z",
          amount: { used: 677, total: 1500, unit: "credits", overage: 12 },
        },
        { key: "chat", label: "Chat", usedPercent: 0, unlimited: true },
        {
          key: "completions",
          label: "Completions",
          usedPercent: 0,
          unlimited: true,
        },
      ],
    },
  ],
};

/** The codex-resets.com news shown in the browser preview. */
export const previewCodexResets: CodexResetsStatus = {
  latestReset: {
    id: "observed-20260825T143200Z",
    announcedAt: "2026-08-25T14:30:00Z",
    resetType: "regular",
    text: "A sample reset announcement.",
    sourceUrl: "https://x.com/example/status/1",
  },
  activeWatch: {
    level: "strong",
    resetChancePercent: 60,
    forecastWindow: "by end of thursday",
    observedAt: "2026-08-27T06:31:31Z",
    expiresAt: "2026-08-28T07:00:00Z",
    text: "A sample watch note hinting at an upcoming reset.",
    sourceUrl: "https://x.com/example/status/2",
  },
  stats: {
    total: 46,
    lastResetAt: "2026-08-25T14:30:00Z",
    daysSinceLast: 1.8,
    avgIntervalDays: 7.6,
  },
  siteUrl: "https://codex-resets.com/",
  fetchedAt: "2026-08-27T09:04:36Z",
};
